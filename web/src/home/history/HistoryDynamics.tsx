...
export const HistoryDynamics: React.FunctionComponent = () => {
  ...
  const onClickDynamic = (id: string) => {
    if (!localState.data.Dynamics[id]) {
      return;
    }
  ...
  return useObserver(() => (
    ...
    >
      ...
      <Stack.Item>{DynamicCard(localState.data, onClickDynamic)}</Stack.Item>
      {DynamicModel(
        localState.data,
        localState.selectId,
        localState.isOpen,
